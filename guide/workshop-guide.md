...
```bash
git clone https://github.com/lesovsky/otelcol-tutorial.git
cd otelcol-tutorial
```

> **Примечание:** Первый запуск `docker compose up -d` загрузит Docker-образы (~2 ГБ). Если интернет на площадке ограничен, рекомендуется загрузить образы заранее:
> ```bash
> docker compose pull
> ```
...
| `bgwriter` | instance | Статистика background writer и checkpoint |
