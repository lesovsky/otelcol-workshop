...
Полный список полей JSON-лога: [PostgreSQL — JSON Logging](https://www.postgresql.org/docs/current/runtime-config-logging.html#RUNTIME-CONFIG-LOGGING-JSONLOG)
...
> **Примечание:** filelog receiver также может читать PostgreSQL-логи в формате CSV (`log_destination = 'csvlog'`), но для этого потребуется отдельная настройка — парсинг через оператор `csv_parser` вместо `json_parser` с указанием имён колонок.
